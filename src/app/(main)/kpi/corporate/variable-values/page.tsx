    ...
    <div className="flex w-full flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <Dropdown><Button variant="tertiary" aria-label="Urutkan"><FunnelSimple className="h-4 w-4" />Urutkan{!(sortBy === 'name' && sortDirection === 'asc') && <><span className="mx-0.5 h-4 w-px bg-border" /><Check className="h-4 w-4" /></>}</Button><Dropdown.Popover><Dropdown.Menu selectedKeys={new Set([`${sortBy}-${sortDirection}`])} selectionMode="single" onAction={(key) => { const option = SORT_OPTIONS.find((item) => `${item.field}-${item.direction}` === String(key)); if (option) updateUrl({ sortBy: option.field, sortDirection: option.direction }); }}>{SORT_OPTIONS.map((option) => <Dropdown.Item key={`${option.field}-${option.direction}`} id={`${option.field}-${option.direction}`} textValue={option.label}>{sortBy === option.field && sortDirection === option.direction && <Check className="h-4 w-4" />}<Label>{option.label}</Label></Dropdown.Item>)}</Dropdown.Menu></Dropdown.Popover></Dropdown>
          {!(sortBy === 'name' && sortDirection === 'asc') && <Button isIconOnly variant="tertiary" aria-label="Hapus pengurutan" onPress={() => updateUrl({ sortBy: 'name', sortDirection: 'asc' })}><X className="h-4 w-4" /></Button>}
        </div>
        <div className="flex items-center gap-2"><Button isIconOnly variant="tertiary" onPress={handleRetry} isDisabled={tableLoading || isSaving} aria-label="Muat ulang nilai variabel"><ArrowsClockwise className={`h-4 w-4 ${tableLoading ? 'animate-spin' : ''}`} /></Button><SearchField aria-label="Cari nilai variabel" value={searchInput} onChange={handleSearchChange} className="w-72"><SearchField.Group><SearchField.SearchIcon /><SearchField.Input placeholder="Cari" /><SearchField.ClearButton aria-label="Hapus pencarian" /></SearchField.Group></SearchField></div>
      </div>
