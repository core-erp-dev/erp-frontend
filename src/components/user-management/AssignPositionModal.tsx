...
export function AssignPositionModal({ user, onClose, onSuccess, onError }: AssignPositionModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await userManagementService.assignUserToPosition(user.userId, parseInt(positionId));
      onSuccess(`${user.fullName || user.email} berhasil ditetapkan ke jabatan ${result.positionName}`);
    } catch (err) {
      onError(err instanceof Error ? err.message : 'Gagal menetapkan jabatan');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Assign Jabatan</h3>
              <p className="text-sm text-gray-500 mt-0.5">Tetapkan pengguna ke jabatan</p>
            </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5">
          <div className="mb-5">
            <label htmlFor="positionId" className="block text-sm font-medium text-gray-700 mb-1.5">
              ID Jabatan
            </label>
            <input
              id="positionId"
            ...
            <p className="text-xs text-gray-400 mt-1.5">
              Jabatan sebelumnya akan otomatis dinonaktifkan
            </p>
          </div>
