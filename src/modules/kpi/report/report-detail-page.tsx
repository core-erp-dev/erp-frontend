...
import { ArrowLeft, DownloadSimple, DotsThreeVertical, House } from '@phosphor-icons/react';
...
export function ReportDetailPage({ id }: { id: string }) {
  ...
  const from = searchParams.get('from') === 'review' ? 'review' : searchParams.get('from') === 'all' ? 'all' : 'mine';
  const listHref = from === 'review' ? '/kpi/report-reviews' : from === 'all' ? '/kpi/reports/all' : '/kpi/reports';
  const listLabel = from === 'review' ? 'Persetujuan Laporan' : from === 'all' ? 'Semua Laporan' : 'Laporan Saya';
